## ============================================================
##  03_Type1Error_S3fix_server.R
##  S3fix 버전 — 기존 결과 건드리지 않음
##  결과 저장: results/T1_S3fix/
##  S3만 실행 (S1/S2는 기존 결과 사용)
##  Usage:
##    Rscript 03_Type1Error_S3fix_server.R --cores 20
##    Rscript 03_Type1Error_S3fix_server.R --test
## ============================================================
...
message(sprintf("Cores: %d | TEST_MODE: %s | S3fix T1", mc_cores, TEST_MODE))

# ── paths ────────────────────────────────────────────────────
base_dir <- "/home2/nekim/Matching2026"
code_dir <- file.path(base_dir, "code")
data_dir <- file.path(base_dir, "data")
res_dir  <- file.path(base_dir, "results", "T1_S3fix")   # ← 별도 저장
dir.create(res_dir, recursive = TRUE, showWarnings = FALSE)

# ── libraries & S3fix generator ──────────────────────────────
source(file.path(code_dir, "00_library_setup.R"))
source(file.path(code_dir, "99_utils.R"))
source(file.path(code_dir, "02_simulation_generator_S3fix.R"))  # ← S3fix
    ...
    for (rate in rates) {
      ...
      pvalues <- mc_apply(seq_len(rep_n), function(h) {
        ...
        per_taxa <- parallel::mclapply(seq_len(dimm), function(x) {
          ...
          main_vec
            ...
            v <- rep(NA_real_, length(COLNAMES$main))
            attr(v, "se") <- rep(NA_real_, length(COLNAMES$se))
            v
          })
        }, mc.cores = 3)
        per_taxa
      ...
      mat <- do.call(rbind,
        lapply(pvalues, function(h_res)
          do.call(rbind, lapply(h_res, function(v) {
            v <- as.numeric(v)
            if (length(v) != length(COLNAMES$main))
              v <- rep(NA_real_, length(COLNAMES$main))
            matrix(v, nrow = 1)
          }))))
      colnames(mat) <- COLNAMES$main

      mat_se <- do.call(rbind,
        lapply(pvalues, function(h_res)
          do.call(rbind, lapply(h_res, function(v) {
            se <- attr(v, "se")
            if (is.null(se) || length(se) != length(COLNAMES$se))
              se <- rep(NA_real_, length(COLNAMES$se))
            matrix(as.numeric(se), nrow = 1)
          }))))
      colnames(mat_se) <- COLNAMES$se
...
message("03_Type1Error_S3fix_server.R complete.")
