## ============================================================
##  04_Power_Calculating.R  [SERVER VERSION]
##  Usage:
##    Rscript 04_Power_Calculating_server.R --cores 20
##    Rscript 04_Power_Calculating_server.R --cores 20 --scenario S1
##    Rscript 04_Power_Calculating_server.R --cores 20 --scenario S1,S2
##    Rscript 04_Power_Calculating_server.R --test
## ============================================================
...
res_dir  <- file.path(base_dir, "results", "Power")
...
for (nsamp in nsamchar) {
  ...
  for (scenario in scenarios) {
    ...
    for (rate in rates) {
      ...
      message(sprintf("  [SAVED] %s | b_levels=%d | ncol_main=%d | ncol_se=%d",
                      tag, length(b),
                      ncol(pvalues_by_b[[1]]),
                      ncol(pvalues_se_by_b[[1]])))

    }  # rate
  }  # scenario
}  # nsamp

message("04_Power_Calculating.R complete.")
