## ============================================================
##  04_Power_S3fix_server.R
##  S3fix 버전 — 기존 결과 건드리지 않음
##  결과 저장: results/Power_S3fix/
##  S3만 실행 (S1/S2는 기존 결과 사용)
##  Usage:
##    Rscript 04_Power_S3fix_server.R --cores 20
##    Rscript 04_Power_S3fix_server.R --test
## ============================================================
...
message(sprintf("Cores: %d | TEST_MODE: %s | S3fix Power", mc_cores, TEST_MODE))

# ── paths ────────────────────────────────────────────────────
base_dir <- "/home2/nekim/Matching2026"
code_dir <- file.path(base_dir, "code")
data_dir <- file.path(base_dir, "data")
res_dir  <- file.path(base_dir, "results", "Power_S3fix")  # ← 별도 저장
...
for (nsamp in nsamchar) {
  ...
  for (scenario in scenarios) {
    ...
    for (rate in rates) {
        ...
        mat <- do.call(rbind,
          lapply(pvalues, function(h_res)
            do.call(rbind, lapply(h_res, function(x_res) {
              ...
              if (length(v) != length(COLNAMES$se))
                v <- rep(NA_real_, length(COLNAMES$se))
              matrix(v, nrow=1)
            }))))
      ...
      message(sprintf("  [SAVED] %s | b_levels=%d", tag, length(b)))

    }  # rate
  }  # scenario
}  # nsamp

message("04_Power_S3fix_server.R complete.")
